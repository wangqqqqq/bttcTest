...
export default class TronContractsBase {
  ...
  public getParentMintERC721TokenContract(token: address, parent: boolean = false) {
    ...
    //return new web3.eth.Contract(this.network.abi('ChildERC721', 'pos'), token)
    return parent
      ? { abi: DummyMintableERC721/*this.network.abi('ChildERC721', 'pos')*/, web3 }
      : new web3.eth.Contract(DummyMintableERC721/*this.network.abi('ChildERC721', 'pos')*/, token)
  }
  ...
  public getParentERC721TokenContract(token: address, parent: boolean = false) {
    ...
    //return new web3.eth.Contract(this.network.abi('ChildERC721', 'pos'), token)
    return parent
        ? { abi: DummyERC721/*this.network.abi('ChildERC721', 'pos')*/, web3 }
        : new web3.eth.Contract(DummyERC721/*this.network.abi('ChildERC721', 'pos')*/, token)
  }
